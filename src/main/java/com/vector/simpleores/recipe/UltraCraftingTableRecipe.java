...
public class UltraCraftingTableRecipe implements Recipe<UltraCraftingTableRecipeInput> {
    ...
    public UltraCraftingTableRecipe(String[] pattern, Map<Character, Ingredient> keys, ItemStack result) {
    ...
    @Override
    public boolean matches(UltraCraftingTableRecipeInput inv, Level level) {
    ...
    @Override
    public ItemStack assemble(UltraCraftingTableRecipeInput pContainer, HolderLookup.Provider pRegistries) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.ULTRA_CRAFTING_TABLE_SERIALIZER.get();
    }
    ...
    public static class Serializer implements RecipeSerializer<UltraCraftingTableRecipe> {
        public static final MapCodec<UltraCraftingTableRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
            ...
            return new UltraCraftingTableRecipe(pattern.toArray(new String[0]), charMap, result);
        }));

        public static final StreamCodec<RegistryFriendlyByteBuf, UltraCraftingTableRecipe> STREAM_CODEC = ByteBufCodecs.fromCodecWithRegistries(CODEC.codec());

        @Override
        public MapCodec<UltraCraftingTableRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, UltraCraftingTableRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}
