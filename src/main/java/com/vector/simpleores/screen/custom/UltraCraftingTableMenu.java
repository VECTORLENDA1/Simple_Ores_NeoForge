...
public class UltraCraftingTableMenu extends AbstractContainerMenu {
    public final UltraCraftingTableEntity blockEntity;
    public final Level level;
    private boolean suppressOutputOnTakeOnce = false;

    public UltraCraftingTableMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(50));
    }

    public UltraCraftingTableMenu(int pContainerId, Inventory inv, BlockEntity Entity, ContainerData data) {
        super(ModMenuTypes.ULTRA_CRAFTING_TABLE_MENU.get(), pContainerId);
        this.blockEntity = ((UltraCraftingTableEntity) Entity);
        ...
        this.addSlot(new SlotItemHandler(this.blockEntity.itemHandler, 49, 181, 72) {
            ...
            public void onTake(Player player, ItemStack stack) {
                ...
                if (recipeOptional.isPresent()) {
                    ...
                    UltraCraftingTableMenu.this.broadcastChanges();
    ...
    public static final int TE_INVENTORY_SLOT_COUNT = 50;  // must be the number of slots you have!
    @Override
    public ItemStack quickMoveStack(Player playerIn, int pIndex) {
        ItemStack sourceStack = ItemStack.EMPTY;
        Slot sourceSlot = this.slots.get(pIndex);
        int teSlotStart = VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT;
        int teOutputSlot = teSlotStart + 49;
        ...
        if (sourceSlot != null && sourceSlot.hasItem()) {
            ...
            if (pIndex == teOutputSlot) {
                ...
                if (recipeOptional.isPresent()) {
                    ...
                    for (int i = 0; i < UltraCraftingTableEntity.INPUT_SLOT.length; i++) {
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.ULTRA_CRAFTING_TABLE.get());
    }

    public void addPlayerInventory(Inventory playerInventory) {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlot(new Slot(playerInventory, j + i * 9 + 9, 28 + j * 18, 160 + i * 18));
            }
        }
    }
    ...
    public void addPlayerHotbar(Inventory playerInventory) {
        for (int i = 0; i < 9; ++i) {
            this.addSlot(new Slot(playerInventory, i, 28 + i * 18, 218));
        }
    }
}